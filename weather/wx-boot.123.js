import {BASEMAPS,loadTile,lonToX,latToY,xToLon,yToLat,clampZoom,MOBILE} from './tiles.js?v=123';
import {createWindLayer,buildIsoPolylines,drawIsoPolylines} from './wind.js?v=123';
import {loadManifest,getHourField,sampleField,fieldStats,preloadCore,packHours,packMeta} from './fields.js?v=123';
import {fetchLiveBundle,extractField,lodForZoom,isRateBlocked} from './meteo.js?v=123';
import {createHazardLayer,refreshHazards} from './hazards.js?v=123';
import {listReports,addReport,reportTypes,advectReports,clearExpired} from './reports.js?v=123';
...
lut:null,lutKey:'',gen:0,raf:0,drag:null,vel:{x:0,y:0},hazards:[],hazOn:true,hoverOn:true,lastHaz:0,capeF:null,precipF:null
};
...
if(el('haz-count'))el('haz-count').textContent=`${nEq} quake · ${nV} volcano · ${nA} alert · ${nR} report`;
}
...
if(typeof isRateBlocked==='function'&&isRateBlocked()){throw new Error('Open-Meteo cooling down — use Pack');}status('Live Open-Meteo (rate-limited)…');
...
windLayer.resize();hazLayer.resize();schedule(true);
}
...
const keys=['temperature_2m','wind_speed_10m','precipitation','pressure_msl','cloud_cover','relative_humidity_2m','cape'];
const lines=[];
for(const k of keys){
try{
...
const meta=LAYERS.find(x=>x.key===k)||{kind:'pct',unitSi:'',name:k};
const toUs=state.units==='us';let v=raw,u=meta.unitSi||'';
if(meta.kind==='temp'){v=toUs?raw*9/5+32:raw;u=toUs?'°F':'°C';}
else if(meta.kind==='wind'){v=toUs?raw*2.236936:raw;u=toUs?'mph':'m/s';}
else if(meta.kind==='precip'){v=toUs?raw/25.4:raw;u=toUs?'in':'mm';}
else if(meta.kind==='pres'){v=toUs?raw*0.02953:raw;u=toUs?'inHg':'hPa';}
lines.push(`${meta.name}: ${v.toFixed(1)} ${u}`);
}catch{}
}
return lines;
}
...
if(!hover)state.probe={lat,lon};
const chip=el('hover-chip');
const box=hover?chip:el('probe');
if(hover){
...
const hz=near.length?('\nNearby: '+near.map(h=>`${h.title||h.kind} ${h.dist.toFixed(0)}km`).join(' · ')):'';
el('probe-meta').textContent=`${L().name}\n${getBasemap().label} · z${state.zoom.toFixed(1)}\nmode ${state.mode}${hz}`;
multiSample(lat,lon).then(lines=>{
if(state.probe&&Math.abs(state.probe.lat-lat)<1e-6)el('probe-meta').textContent=lines.join('\n')+hz+'\n'+getBasemap().label;
}).catch(()=>{});
}
function initUI(){

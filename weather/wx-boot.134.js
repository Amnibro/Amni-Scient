import {BASEMAPS,loadTile,lonToX,latToY,xToLon,yToLat,clampZoom,MOBILE} from './tiles.js?v=134';
import {createWindLayer,buildIsoPolylines,drawIsoPolylines} from './wind.js?v=134';
import {loadManifest,getHourField,sampleField,fieldStats,preloadCore,packHours,packMeta,enhanceField,upsampleBilinear,smoothBox} from './fields.js?v=134';
import {fetchLiveBundle,extractField,lodForZoom,isRateBlocked,fetchPointForecast} from './meteo.js?v=134';
import {createHazardLayer,refreshHazards} from './hazards.js?v=134';
import {listReports,addReport,reportTypes,advectReports,clearExpired} from './reports.js?v=134';
import {createRefLayer,loadBorders,projectScreen} from './ref.js?v=134';
import {renderForecastPanel,setForecastLoading,setForecastError} from './forecast.js?v=134';
...
const m=String(msg||'');
if(/429|rate limited|cooling down|switch to Pack|use Pack|API busy|busy/i.test(m))return 'Forecast delayed (API limit) — try again shortly';
return m;
}
const status=(t,opts={})=>{
...
try{
if(typeof isRateBlocked==='function'&&isRateBlocked())throw new Error('Live map API busy — retry shortly');
const key=viewLiveKey();
let bundle=state.liveBundle;
const force=!!opts.force;
if(force||!bundle||state.liveKey!==key){
bundle=await fetchLiveBundle({lat:state.lat,lon:state.lon,zoom:state.zoom,activeKey:L().key,onStatus:status});
...
status(`<span class="ok">Live ${lod} · ${bundle.lats.length} pts · ${field.w}×${field.h}</span>`);
}catch(liveErr){
console.warn(liveErr);
status(`<span class="warn">${softApiMsg(liveErr.message||liveErr)} · showing global fields</span>`);
await loadPackLayer();
}
...
if(state.mode!=='live'&&!state.wantLive)return;
const key=viewLiveKey();
if(key!==state.liveKey)refresh({force:true});
else if(state.livePts){applyField(fieldFromPoints(state.livePts,state.zoom));schedule(true);}
},450);
}

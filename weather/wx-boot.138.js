...
const state={
layer:0,preset:'thermal',opacity:0.7,reverse:false,autorange:true,units:(_prefs.units==='si'?'si':'us'),
mode:'pack',tIndex:0,hours:[],field:null,vmin:0,vmax:1,gamma:1,soft:null,
stops:PRESETS.thermal.map(c=>c.slice()),basemap:'cartoDark',
lat:Number.isFinite(+_prefs.lat)?+_prefs.lat:20,lon:Number.isFinite(+_prefs.lon)?+_prefs.lon:0,zoom:Number.isFinite(+_prefs.zoom)?+_prefs.zoom:2.2,probe:null,panning:false,windOn:true,isoOn:false,
...
placeLabel:(_prefs.place||''),sugIdx:-1,sugHits:[],geoDone:!!(Number.isFinite(+_prefs.lat)&&Number.isFinite(+_prefs.lon)),citiesOn:true,bordersOn:true,pinOn:true,probeInfo:null,
fcData:null,fcTab:'summary',fcOpen:false,fcTimer:0,fcGen:0,sheet:null,statusT:0,pinching:false,pinch:null,ptrs:new Map(),livePts:null,liveBundle:null,liveKey:'',liveTimer:0,smoothOn:true,wantLive:false
};
const el=id=>document.getElementById(id);
function softApiMsg(msg){
const m=String(msg||'');
if(/429|rate limited|cooling down|switch to Pack|use Pack|API busy|busy/i.test(m))return 'API rate limited — map keeps working on global fields';
return m;
}
function setLiveChip(on,text){
const c=el('live-chip');if(!c)return;
if(!on){c.hidden=true;c.classList.remove('on');return;}
c.hidden=false;c.classList.add('on');
const t=el('live-chip-text');if(t)t.textContent=text||'Live loading…';
}
function showLoader(phase,meta,pct){
const root=el('wx-loader');if(!root)return;
root.style.display='flex';root.classList.add('on');root.classList.remove('out');root.setAttribute('aria-busy','true');
const p=el('wx-load-phase');if(p)p.textContent=phase||'Loading';
const m=el('wx-load-meta');if(m)m.innerHTML=meta||'Working…';
const fill=el('wx-load-fill');if(fill&&pct!=null)fill.style.width=Math.max(0,Math.min(100,pct))+'%';
const arc=el('wx-load-arc');if(arc&&pct!=null){const c=534;arc.style.strokeDashoffset=String(c*(1-Math.max(0,Math.min(1,pct/100))));}
const skip=el('wx-load-skip');if(skip)skip.hidden=false;
}
function hideLoader(){
const root=el('wx-loader');if(!root)return;
root.classList.add('out');root.classList.remove('on');root.setAttribute('aria-busy','false');
setTimeout(()=>{if(root.classList.contains('out'))root.style.display='none';},420);
const skip=el('wx-load-skip');if(skip)skip.hidden=true;
}
function liveProgress(msg,pct){
const plain=String(msg||'').replace(/<[^>]+>/g,'');
setLiveChip(true,plain);
const fill=el('wx-load-fill');if(fill&&pct!=null)fill.style.width=Math.max(0,Math.min(100,pct))+'%';
const arc=el('wx-load-arc');if(arc&&pct!=null){const c=534;arc.style.strokeDashoffset=String(c*(1-Math.max(0,Math.min(1,pct/100))));}
const p=el('wx-load-phase');if(p)p.textContent='Live';
const m=el('wx-load-meta');if(m)m.textContent=plain+(pct!=null?` · ${Math.round(pct)}%`:'');
status(msg);
}
const status=(t,opts={})=>{
...
const meta=L();
const[cLo,cHi]=clampFieldRange(state.vmin,state.vmax,meta);
state.vmin=cLo;state.vmax=cHi;
const lo=formatVal(cLo,meta),hi=formatVal(cHi,meta);
let loV=lo.v,hiV=hi.v;
if(meta.kind!=='temp'&&meta.kind!=='pres'&&loV<0)loV=0;
el('leg-title').textContent=meta.name;
el('leg-lo').textContent=`${loV.toFixed(1)} ${lo.u}`;
el('leg-hi').textContent=`${hiV.toFixed(1)} ${hi.u}`;
const cue=el('leg-cue');if(cue)cue.textContent=KIND_CUE[meta.kind]||'';
const mid=el('leg-mid');if(mid){const mv=(cLo+cHi)/2;const mf=formatVal(mv,meta);mid.title=`mid ${mf.v.toFixed(1)} ${mf.u}`;}
legendBar();
el('attr').textContent=`${getBasemap().attr} · Pack · Amni-Weather`;
}
function kindFloor(kind){return kind==='temp'?-90:kind==='pres'?870:0;}
function kindCeil(kind){return kind==='temp'?60:kind==='pres'?1100:kind==='pct'?100:kind==='uv'?20:kind==='vis'?200000:kind==='wind'?120:kind==='precip'?200:kind==='cape'?6000:1e9;}
function clampFieldRange(vmin,vmax,meta){const k=meta?.kind||'';let lo=+vmin,hi=+vmax;if(!Number.isFinite(lo))lo=meta?.barMin??0;if(!Number.isFinite(hi))hi=meta?.barMax??1;const fl=kindFloor(k),ce=kindCeil(k);lo=Math.max(fl,lo);hi=Math.min(ce,Math.max(hi,lo));if(k==='pct'){lo=Math.max(0,lo);hi=Math.min(100,Math.max(hi,lo+0.1));}if(k==='vis'||k==='wind'||k==='precip'||k==='cape'||k==='uv')lo=Math.max(0,lo);if(hi<=lo)hi=lo+(Math.abs(lo)*0.05||1);return[lo,hi];}
...
const meta=L();
if(state.autorange){
const st=fieldStats(packField);const span=Math.max(0,st[1]-st[0]);const pad=span*0.05||(Math.abs(st[1])*0.02)||0.5;
let lo=st[0]-pad,hi=st[1]+pad;
if(meta.kind!=='temp'&&meta.kind!=='pres')lo=Math.max(0,st[0]>0?Math.max(0,st[0]-pad):0);
if(meta.kind==='temp')lo=st[0]-pad;
if(meta.kind==='pres'){lo=Math.max(870,st[0]-pad);hi=Math.min(1100,st[1]+pad);}
[lo,hi]=clampFieldRange(lo,hi,meta);
state.vmin=lo;state.vmax=hi;
if(el('rmin'))el('rmin').value=String(+lo.toFixed(2));
if(el('rmax'))el('rmax').value=String(+hi.toFixed(2));
}else{
let lo=+el('rmin').value,hi=+el('rmax').value;
[lo,hi]=clampFieldRange(lo,hi,meta);
state.vmin=lo;state.vmax=hi;
}
updateLegend();
...
const key=L().key;
const raw=await getHourField(key,state.tIndex);
const f=enhanceField(raw,{scale:5,maxW:PERF.mobile?400:720,maxH:PERF.mobile?200:360,smooth:state.smoothOn?3:1});
applyField(f);
const uRaw=await getHourField('wind_u',state.tIndex);
const vRaw=await getHourField('wind_v',state.tIndex);
const u=enhanceField(uRaw,{scale:4,maxW:PERF.mobile?320:560,maxH:PERF.mobile?160:280,smooth:2});
const v=enhanceField(vRaw,{scale:4,maxW:PERF.mobile?320:560,maxH:PERF.mobile?160:280,smooth:2});
...
status(`<span class="ok">Pack ${raw.w}×${raw.h} → ${f.w}×${f.h} smooth · ${hours.length}h</span>`);
}
function idwRaster(pts,w,h,b,opts={}){
const power=opts.power||1.6,kMax=opts.k||Math.min(12,pts.lats.length),n=pts.lats.length,data=new Float32Array(w*h);
const lat0=b.lat0,lat1=b.lat1,lon0=b.lon0,lon1=b.lon1;
const hDen=Math.max(1,h-1),wDen=Math.max(1,w-1);
const ds=new Float64Array(n),ix=new Uint16Array(n);
for(let j=0;j<h;j++){
const lat=lat1+(lat0-lat1)*(j/hDen);
const cos=Math.cos(lat*Math.PI/180)||1e-6;
for(let i=0;i<w;i++){
const lon=lon0+(lon1-lon0)*(i/wDen);
for(let k=0;k<n;k++){
let dlon=pts.lons[k]-lon;if(dlon>180)dlon-=360;if(dlon<-180)dlon+=360;
const dlat=pts.lats[k]-lat;
ds[k]=dlat*dlat+(dlon*cos)*(dlon*cos);ix[k]=k;
}
const use=Math.min(kMax,n);
for(let a=0;a<use;a++){let bi=a;for(let b0=a+1;b0<n;b0++)if(ds[b0]<ds[bi])bi=b0;if(bi!==a){const td=ds[a];ds[a]=ds[bi];ds[bi]=td;const ti=ix[a];ix[a]=ix[bi];ix[bi]=ti;}}
let wsum=0,vsum=0;
if(ds[0]<1e-12){data[j*w+i]=pts.vals[ix[0]];continue;}
for(let a=0;a<use;a++){const ww=1/Math.pow(ds[a]+1e-8,power/2);wsum+=ww;vsum+=ww*pts.vals[ix[a]];}
data[j*w+i]=wsum>0?vsum/wsum:pts.vals[ix[0]];
}
}
return data;
}
function liveRasterSize(zoom,b){
const spanLon=Math.max(1,(b.lon1-b.lon0+360)%360||360),spanLat=Math.max(1,b.lat1-b.lat0);
const z=Math.max(1,zoom||3);
const dens=PERF.mobile?(z<5?1.1:1.6):(z<4?1.4:z<7?2.1:2.8);
const w=Math.min(PERF.mobile?280:560,Math.max(160,Math.round((spanLon>350?360:spanLon)*dens)));
const h=Math.min(PERF.mobile?140:280,Math.max(80,Math.round(spanLat*dens)));
return{w,h};
}
function fieldFromPoints(pts,zoom){
const b=pts.b||{lat0:-85,lat1:85,lon0:-180,lon1:180};
const{w,h}=liveRasterSize(zoom,b);
let data=idwRaster(pts,w,h,b,{power:1.55,k:PERF.mobile?8:12});
data=smoothBox(data,w,h,1);
data=smoothBox(data,w,h,1);
const upW=Math.min(PERF.mobile?360:720,w*2),upH=Math.min(PERF.mobile?180:360,h*2);
if(upW>w||upH>h){data=upsampleBilinear(data,w,h,upW,upH);return{data,w:upW,h:upH,lat0:b.lat0,lat1:b.lat1,lon0:b.lon0,lon1:b.lon1};}
return{data,w,h,lat0:b.lat0,lat1:b.lat1,lon0:b.lon0,lon1:b.lon1};
}
function viewLiveKey(){
const lod=lodForZoom(state.zoom);
const qLat=Math.round(state.lat*2)/2,qLon=Math.round(state.lon*2)/2,qZ=Math.round(state.zoom*2)/2;
return lod.name+'|'+qZ+'|'+qLat+'|'+qLon+'|'+L().key;
}
async function applyLiveBundle(bundle){
const tix=Math.min(state.tIndex,Math.max(0,(bundle.hours?.length||1)-1));
const pts=extractField(bundle,L().key,tix);
state.livePts=pts;
const field=fieldFromPoints(pts,state.zoom);
applyField(field);
try{
const wu=extractField(bundle,'wind_speed_10m',tix);
const wd=extractField(bundle,'wind_direction_10m',tix);
const windF=fieldFromPoints(wu,state.zoom);
const dirF=fieldFromPoints(wd,state.zoom);
const ww=windF.w,hh=windF.h,U=new Float32Array(ww*hh),V=new Float32Array(ww*hh);
for(let i=0;i<ww*hh;i++){const sp=windF.data[i]||0,dir=((dirF.data[i]||0)+180)*Math.PI/180;U[i]=sp*Math.sin(dir);V[i]=sp*Math.cos(dir);}
state.windU=U;state.windV=V;windLayer.setFields(U,V,ww,hh,{lat0:windF.lat0,lat1:windF.lat1,lon0:windF.lon0,lon1:windF.lon1});
}catch{}
state.hours=bundle.hours||[];
el('t-slider').max=String(Math.max(0,state.hours.length-1));
el('t-label').textContent=state.hours[state.tIndex]||`h${state.tIndex}`;
state.mode='live';
return field;
}
...
const step=(state.panning||state.pinching)?(PERF.mobile?4:3):(PERF.mobile?2:1);
...
try{
const data=await fetchPointForecast(t.lat,t.lon,{days:7,marine:false,air:false});
...
state.placeLabel=label||'';state.geoDone=true;
if(el('q')&&label)el('q').value=label;
savePrefs({lat,lon,zoom:state.zoom,place:state.placeLabel,units:state.units});
...
const US_NAME={};Object.keys(US_ST).forEach(k=>{US_NAME[US_ST[k].toLowerCase()]=US_ST[k];});
const CA_ST={ab:'Alberta',bc:'British Columbia',mb:'Manitoba',nb:'New Brunswick',nl:'Newfoundland and Labrador',ns:'Nova Scotia',nt:'Northwest Territories',nu:'Nunavut',on:'Ontario',pe:'Prince Edward Island',qc:'Quebec',sk:'Saskatchewan',yt:'Yukon'};
const CA_NAME={};Object.keys(CA_ST).forEach(k=>{CA_NAME[CA_ST[k].toLowerCase()]=CA_ST[k];});
function resolveAdmin(tok){if(!tok)return null;const t=tok.trim().toLowerCase().replace(/\./g,'');if(US_ST[t])return{admin:US_ST[t],cc:'US'};if(US_NAME[t])return{admin:US_NAME[t],cc:'US'};if(CA_ST[t])return{admin:CA_ST[t],cc:'CA'};if(CA_NAME[t])return{admin:CA_NAME[t],cc:'CA'};return null;}
const CC_MAP={us:'US',usa:'US','united states':'US','united states of america':'US',ca:'CA',canada:'CA',uk:'GB',gb:'GB','united kingdom':'GB',england:'GB',scotland:'GB',wales:'GB',au:'AU',australia:'AU',nz:'NZ','new zealand':'NZ',jp:'JP',japan:'JP',de:'DE',germany:'DE',fr:'FR',france:'FR',es:'ES',spain:'ES',it:'IT',italy:'IT',mx:'MX',mexico:'MX',br:'BR',brazil:'BR',in:'IN',india:'IN',cn:'CN',china:'CN',kr:'KR','south korea':'KR',korea:'KR',se:'SE',sweden:'SE',no:'NO',norway:'NO',fi:'FI',finland:'FI',dk:'DK',denmark:'DK',nl:'NL',netherlands:'NL',be:'BE',belgium:'BE',ch:'CH',switzerland:'CH',at:'AT',austria:'AT',pl:'PL',poland:'PL',pt:'PT',portugal:'PT',ie:'IE',ireland:'IE',za:'ZA','south africa':'ZA',ar:'AR',argentina:'AR',cl:'CL',chile:'CL',co:'CO',colombia:'CO',pe:'PE',peru:'PE',sg:'SG',singapore:'SG',th:'TH',thailand:'TH',ph:'PH',philippines:'PH',id:'ID',indonesia:'ID',my:'MY',malaysia:'MY',ae:'AE',uae:'AE','united arab emirates':'AE',sa:'SA','saudi arabia':'SA',il:'IL',israel:'IL',tr:'TR',turkey:'TR',türkiye:'TR',ru:'RU',russia:'RU',ua:'UA',ukraine:'UA',eg:'EG',egypt:'EG',ng:'NG',nigeria:'NG',ke:'KE',kenya:'KE',gr:'GR',greece:'GR',cz:'CZ','czech republic':'CZ',czechia:'CZ',ro:'RO',romania:'RO',hu:'HU',hungary:'HU',is:'IS',iceland:'IS'};
function resolveCountry(tok){if(!tok)return null;const t=tok.trim().toLowerCase().replace(/\./g,'');return CC_MAP[t]||null;}
function parsePlaceQuery(q){let s=String(q||'').trim().replace(/\s+/g,' ');if(!s)return{city:'',admin:null,cc:null,variants:[]};const tail=s.match(/,?\s*([A-Za-zÀ-ÿ .]{2,40})$/);let cc=null;if(tail){const c=resolveCountry(tail[1]);if(c){cc=c;s=s.slice(0,s.length-tail[0].length).trim().replace(/,\s*$/,'');}}const parts=s.split(/,\s*/).map(x=>x.trim()).filter(Boolean);let city=parts[0]||s,admin=null;if(parts.length>=2){const a=resolveAdmin(parts[1]);if(a){admin=a.admin;cc=cc||a.cc;}else{const c2=resolveCountry(parts[1]);if(c2)cc=cc||c2;else admin=parts[1];}if(parts.length>=3){const c=resolveCountry(parts[parts.length-1]);if(c)cc=c;}}if(parts.length===1){const m=s.match(/^(.+?)\s+([A-Za-z]{2})$/);if(m){const a=resolveAdmin(m[2]);if(a){city=m[1].trim();admin=a.admin;cc=cc||a.cc;}else{const c=resolveCountry(m[2]);if(c){city=m[1].trim();cc=cc||c;}}}else{const low=s.toLowerCase();const names=Object.keys(US_NAME).concat(Object.keys(CA_NAME)).sort((a,b)=>b.length-a.length);for(const n of names){if(low.endsWith(' '+n)){city=s.slice(0,-(n.length+1)).trim();const a=resolveAdmin(n);if(a){admin=a.admin;cc=cc||a.cc;}break;}}}}const variants=[];if(city)variants.push(city);if(s&&s!==city)variants.push(s);if(city&&admin)variants.push(city+' '+admin);return{city,admin,cc,variants:[...new Set(variants.filter(Boolean))]};}
function rankHits(hits,pq){const city=(pq.city||'').toLowerCase(),admin=(pq.admin||'').toLowerCase(),cc=(pq.cc||'').toUpperCase();return hits.map(h=>{let sc=0;const hn=(h.name||'').toLowerCase(),ha=(h.admin1||'').toLowerCase();if(hn===city)sc+=80;else if(hn.startsWith(city))sc+=40;else if(hn.includes(city))sc+=15;if(admin&&ha===admin.toLowerCase())sc+=120;else if(admin&&ha.includes(admin))sc+=50;if(cc&&(h.country_code||'').toUpperCase()===cc)sc+=35;if(h.feature_code==='PPLC')sc+=25;else if(h.feature_code==='PPLA')sc+=15;else if((h.feature_code||'').startsWith('PPL'))sc+=8;if(h.population)sc+=Math.min(25,Math.log10(h.population+1)*4);return{h,sc};}).sort((a,b)=>b.sc-a.sc).map(x=>x.h);}
let geoAbort=null,geoTimer=0;
async function geocodeRaw(name,count=8,opts={},signal){
const p=new URLSearchParams({name,count:String(count),language:'en',format:'json'});
if(opts.countryCode)p.set('countryCode',opts.countryCode);
const r=await fetch('https://geocoding-api.open-meteo.com/v1/search?'+p.toString(),{signal});
const j=await r.json();
return j.results||[];
}
async function geocode(q,count=8){
if(geoAbort)geoAbort.abort();
geoAbort=new AbortController();
const signal=geoAbort.signal;
const pq=parsePlaceQuery(q);
const seen=new Set();
let hits=[];
const push=list=>{for(const h of list||[]){const id=h.id??`${h.latitude},${h.longitude},${h.name}`;if(seen.has(id))continue;seen.add(id);hits.push(h);}};
const tries=[];
if(pq.city){tries.push([pq.city,pq.cc?{countryCode:pq.cc}:{}]);if(pq.cc)tries.push([pq.city,{}]);}
for(const v of pq.variants){if(v!==pq.city)tries.push([v,pq.cc?{countryCode:pq.cc}:{}]);}
tries.push([q,{}]);
for(const [name,opts] of tries){
if(!name||name.length<2)continue;
try{push(await geocodeRaw(name,count,opts,signal));}catch(e){if(e.name==='AbortError')throw e;}
if(hits.length>=3)break;
}
if(!hits.length&&pq.city){
try{push(await geocodeRaw(pq.city,count,{},signal));}catch(e){if(e.name==='AbortError')throw e;}
}
return rankHits(hits,pq).slice(0,count);
}
...
try{
const hits=await geocode(q,8);
if(!hits.length){status('<span class="warn">No match — try city name</span>');hideSuggest();return;}
const label=hits[0].name+(hits[0].admin1?', '+hits[0].admin1:'');
if(hits.length===1){goToPlace(hits[0].latitude,hits[0].longitude,label,hits[0]);hideSuggest();return;}
renderSuggest(hits);
goToPlace(hits[0].latitude,hits[0].longitude,label,hits[0]);
...
function applyUserPlace(lat,lon,label,opts={}){
state.lat=lat;state.lon=lon;state.zoom=Math.max(state.zoom,opts.zoom||7.2);state.geoDone=true;state.placeLabel=label||'Your location';
if(el('q')&&state.placeLabel)el('q').value=state.placeLabel;
savePrefs({lat,lon,zoom:state.zoom,place:state.placeLabel,units:state.units});
status(`<span class="ok">${state.placeLabel} · located</span>`);
schedule(true);
if(opts.probe!==false)showProbe(lat,lon,{label:state.placeLabel,skipRev:true,keepPlace:true});
else{state.probe={lat,lon};updateLocMarker();}
}
async function locateByIp(opts={}){
...
const reg=j.region||j.region_name||j.principalSubdivision||'';
const ctry=j.country||j.country_name||'';
const label=city?(reg&&reg!==city?`${city}, ${reg}`:city+(ctry?`, ${ctry}`:'')):(reg||ctry||'Your area');
applyUserPlace(lat,lon,label,{...opts,zoom:opts.zoom||6.8,probe:opts.probe});
return true;
}catch{}
}
return false;
...
const done=ok=>{if(btn){btn.classList.remove('locating');if(ok)btn.classList.add('located');}};
const tryIp=async()=>{const ok=await locateByIp(opts);done(ok);if(!ok&&!opts.silent)status('<span class="warn">Location unavailable — search any city worldwide</span>');};
if(!navigator.geolocation){tryIp();return;}
navigator.geolocation.getCurrentPosition(
async p=>{
const lat=p.coords.latitude,lon=p.coords.longitude;
let label=(await reverseLabel(lat,lon))||'Your location';
applyUserPlace(lat,lon,label,{...opts,zoom:opts.zoom||7.2});
done(true);
},
()=>{tryIp();},
{enableHighAccuracy:false,timeout:opts.timeout||8000,maximumAge:opts.maxAge??300000}
);
}
...
el('autorange').onchange=e=>{state.autorange=e.target.checked;if(state.field){applyField(state.field);schedule(true);}};el('rmin')&&(el('rmin').oninput=()=>{if(!state.autorange&&state.field){applyField(state.field);schedule(true);}});el('rmax')&&(el('rmax').oninput=()=>{if(!state.autorange&&state.field){applyField(state.field);schedule(true);}});
el('smooth')&&(el('smooth').checked=state.smoothOn,el('smooth').onchange=e=>{state.smoothOn=e.target.checked;if(state.mode==='pack')loadPackLayer().then(()=>schedule(true));else if(state.livePts){applyField(fieldFromPoints(state.livePts,state.zoom));schedule(true);}});
...
const iOn=el('iso-on');if(iOn){iOn.checked=state.isoOn;iOn.onchange=async e=>{state.isoOn=e.target.checked;if(state.isoOn){const p=await getHourField('pressure_msl',state.tIndex);state.isoLines=buildIsoPolylines(p.data,p.w,p.h,{lat0:p.lat0,lat1:p.lat1,lon0:p.lon0,lon1:p.lon1},6);}else state.isoLines=null;schedule(true);};}
function syncUnitsUi(){const root=el('units');if(!root)return;[...root.children].forEach(x=>x.classList.toggle('active',x.dataset.u===state.units));}
syncUnitsUi();
el('units').onclick=e=>{const b=e.target.closest('button[data-u]');if(!b)return;state.units=b.dataset.u;savePrefs({units:state.units});syncUnitsUi();updateLegend();if(state.probe)showProbe(state.probe.lat,state.probe.lon,{skipRev:true,keepPlace:true,label:state.placeLabel,openSheet:false});if(state.fcData)paintForecast();};
...
el('t-slider').oninput=e=>{state.tIndex=+e.target.value;if(state.mode==='live'&&state.liveBundle)applyLiveBundle(state.liveBundle).then(()=>schedule(true));else loadPackLayer().then(()=>schedule(true));};
el('t-prev').onclick=()=>{state.tIndex=Math.max(0,state.tIndex-1);el('t-slider').value=state.tIndex;if(state.mode==='live'&&state.liveBundle)applyLiveBundle(state.liveBundle).then(()=>schedule(true));else loadPackLayer().then(()=>schedule(true));};
el('t-next').onclick=()=>{const mx=+el('t-slider').max;state.tIndex=Math.min(mx,state.tIndex+1);el('t-slider').value=state.tIndex;if(state.mode==='live'&&state.liveBundle)applyLiveBundle(state.liveBundle).then(()=>schedule(true));else loadPackLayer().then(()=>schedule(true));};
el('t-play').onclick=(()=>{let t=null;return()=>{if(t){clearInterval(t);t=null;el('t-play').textContent='Play';return;}el('t-play').textContent='Pause';t=setInterval(()=>{const mx=+el('t-slider').max;state.tIndex=state.tIndex>=mx?0:state.tIndex+1;el('t-slider').value=state.tIndex;if(state.mode==='live'&&state.liveBundle)applyLiveBundle(state.liveBundle).then(()=>schedule(true));else loadPackLayer().then(()=>schedule(true));},600);};})();
...
coasting=true;
const coast=()=>{if(state.drag||state.pinching||!coasting||Math.hypot(state.vel.y,state.vel.x)<0.35){coasting=false;state.panning=false;try{windLayer.onViewChange&&windLayer.onViewChange();}catch{}schedule(true);scheduleLiveLod();return;}
...
showLoader('Boot','Loading basemap + field pack…',8);
const skip=el('wx-load-skip');
if(skip){skip.hidden=false;skip.onclick=()=>{state.wantLive=false;hideLoader();status('<span class="ok">Using global fields</span>');};}
loadBorders('/weather/data/').then(()=>schedule(true)).catch(()=>{});
try{
showLoader('Pack','Reading precomputed fields…',18);
...
console.error(e);
status(`<span class="warn">Boot issue: ${e.message||e}</span>`);
hideLoader();
}
if(state.placeLabel&&el('q')&&!el('q').value)el('q').value=state.placeLabel;
if(state.geoDone&&Number.isFinite(state.lat)&&Number.isFinite(state.lon)){
status(`<span class="ok">${state.placeLabel||'Saved place'} · restored</span>`);
schedule(true);
}else{
locateUser({probe:false,zoom:6.5,timeout:5000,maxAge:300000,silent:true});
}
})();

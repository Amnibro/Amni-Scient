import {BASEMAPS,loadTile,lonToX,latToY,xToLon,yToLat,clampZoom,MOBILE} from './tiles.js?v=133';
import {createWindLayer,buildIsoPolylines,drawIsoPolylines} from './wind.js?v=133';
import {loadManifest,getHourField,sampleField,fieldStats,preloadCore,packHours,packMeta} from './fields.js?v=133';
import {fetchLiveBundle,extractField,lodForZoom,isRateBlocked,fetchPointForecast} from './meteo.js?v=133';
import {createHazardLayer,refreshHazards} from './hazards.js?v=133';
import {listReports,addReport,reportTypes,advectReports,clearExpired} from './reports.js?v=133';
import {createRefLayer,loadBorders,projectScreen} from './ref.js?v=133';
import {renderForecastPanel,setForecastLoading,setForecastError} from './forecast.js?v=133';
...
status(`<span class="ok">Global fields ${f.w}×${f.h} · ${hours.length}h</span>`);
}
function idwRaster(pts,w,h,b,opts={}){
const power=opts.power||2,n=pts.lats.length,data=new Float32Array(w*h);
...
let wsum=0,vsum=0,best=0,bd=1e18;
for(let k=0;k<n;k++){
let dlon=pts.lons[k]-lon;if(dlon>180)dlon-=360;if(dlon<-180)dlon+=360;
const dlat=pts.lats[k]-lat;
const d2=dlat*dlat+(dlon*cos)*(dlon*cos);
if(d2<bd){bd=d2;best=pts.vals[k];}
if(d2<1e-12){wsum=1;vsum=pts.vals[k];break;}
const ww=1/Math.pow(d2,power/2);
wsum+=ww;vsum+=ww*pts.vals[k];
}
data[j*w+i]=wsum>0?vsum/wsum:best;
}
}
return data;
}
...
const dens=PERF.mobile?0.55:1;
const spanLon=Math.max(1,(pts.b.lon1-pts.b.lon0+360)%360||360),spanLat=Math.max(1,pts.b.lat1-pts.b.lat0);
const w=Math.min(PERF.mobile?200:360,Math.max(120,Math.round((spanLon>350?360:spanLon)*dens*(state.zoom<5?1:1.35))));
const h=Math.min(PERF.mobile?100:180,Math.max(60,Math.round(spanLat*dens*(state.zoom<5?1:1.35))));
const data=idwRaster(pts,w,h,pts.b,{power:2.15});
...
const ru=idwRaster(wu,w,h,pts.b,{power:2}),rd=idwRaster(wd,w,h,pts.b,{power:2});
const U=new Float32Array(w*h),V=new Float32Array(w*h);
for(let i=0;i<w*h;i++){const sp=ru[i]||0,dir=((rd[i]||0)+180)*Math.PI/180;U[i]=sp*Math.sin(dir);V[i]=sp*Math.cos(dir);}
state.windU=U;state.windV=V;windLayer.setFields(U,V,w,h,{lat0:pts.b.lat0,lat1:pts.b.lat1,lon0:pts.b.lon0,lon1:pts.b.lon1});
}catch{}
...
status(`<span class="ok">Live ${bundle.lats.length} stns · ${w}×${h}</span>`);
}catch(liveErr){

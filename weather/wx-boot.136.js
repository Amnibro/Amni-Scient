import {BASEMAPS,loadTile,lonToX,latToY,xToLon,yToLat,clampZoom,MOBILE} from './tiles.js?v=136';
import {createWindLayer,buildIsoPolylines,drawIsoPolylines} from './wind.js?v=136';
import {loadManifest,getHourField,sampleField,fieldStats,preloadCore,packHours,packMeta,enhanceField,upsampleBilinear,smoothBox} from './fields.js?v=136';
import {fetchLiveBundle,extractField,lodForZoom,isRateBlocked,fetchPointForecast,circuitRemaining} from './meteo.js?v=136';
import {createHazardLayer,refreshHazards} from './hazards.js?v=136';
import {listReports,addReport,reportTypes,advectReports,clearExpired} from './reports.js?v=136';
import {createRefLayer,loadBorders,projectScreen} from './ref.js?v=136';
import {renderForecastPanel,setForecastLoading,setForecastError} from './forecast.js?v=136';
...
mode:'live',tIndex:0,hours:[],field:null,vmin:0,vmax:1,gamma:1,soft:null,
...
fcData:null,fcTab:'summary',fcOpen:false,fcTimer:0,fcGen:0,sheet:null,statusT:0,pinching:false,pinch:null,ptrs:new Map(),livePts:null,liveBundle:null,liveKey:'',liveTimer:0,smoothOn:true,wantLive:true
};
...
legendBar();
el('attr').textContent=`${getBasemap().attr} · Live · Amni-Weather`;
}
...
try{
if(!state.field||opts.packFirst!==false){
if(boot)showLoader('Pack','Loading global fields…',12);
try{await loadPackLayer();schedule(true);if(boot)showLoader('Pack','Global fields ready',35);}catch(pe){console.warn(pe);}
}
if(isRateBlocked()){
const waitSec=Math.ceil((typeof circuitRemaining==='function'?circuitRemaining():60000)/1000);
status(`<span class="warn">API cooling down ~${waitSec}s · map on global fields</span>`,{hold:5000});
setLiveChip(false);
if(boot)hideLoader();
...
if(!quiet){
if(boot)showLoader('Live','Contacting Open-Meteo…',45);
else setLiveChip(true,'Fetching live stations…');
}
try{
const key=viewLiveKey();
const onStatus=m=>{
const mm=String(m||'');
...
let bundle=state.liveBundle;
if(force||!bundle||state.liveKey!==key){
bundle=await fetchLiveBundle({lat:state.lat,lon:state.lon,zoom:state.zoom,activeKey:L().key,onStatus,force});
state.liveBundle=bundle;state.liveKey=key;
}
const field=await applyLiveBundle(bundle);
const lod=bundle.lod?.name||lodForZoom(state.zoom).name;
status(`<span class="ok">Live ${lod} · ${bundle.lats.length} stations · ${field.w}×${field.h}</span>`);
setLiveChip(false);
if(boot){showLoader('Ready','Live fields online',100);setTimeout(hideLoader,350);}
}catch(liveErr){
console.warn(liveErr);
status(`<span class="warn">${softApiMsg(liveErr.message||liveErr)} · global fields</span>`,{hold:5000});
setLiveChip(false);
if(!state.field)await loadPackLayer();
if(boot)hideLoader();
}
if(state.hazOn)reloadHazards().catch(()=>{});
schedule(true);
...
setLiveChip(false);
try{await loadPackLayer();schedule(true);}catch{}
if(boot)hideLoader();
}
}
...
const key=viewLiveKey();
if(state.livePts&&state.liveKey&&state.liveKey.split('|')[0]===key.split('|')[0]){
if(state.mode==='live'){applyField(fieldFromPoints(state.livePts,state.zoom));schedule(true);}
return;
}
if(key!==state.liveKey)refresh({force:false,quiet:true,packFirst:false});
},1800);
}
...
function parsePlaceQuery(q){let s=String(q||'').trim().replace(/\s+/g,' ');if(!s)return{city:'',admin:null,cc:null,variants:[]};s=s.replace(/,?\s*(USA|U\.S\.A\.|U\.S\.|United States of America|United States|US|Canada|CA|UK|United Kingdom|Australia|AU)$/i,'').trim();const parts=s.split(/,\s*/).map(x=>x.trim()).filter(Boolean);let city=parts[0]||s,admin=null,cc=null;if(parts.length>=2){const a=resolveAdmin(parts[1]);if(a){admin=a.admin;cc=a.cc;}else{admin=parts[1];}if(parts.length>=3){const c=parts[parts.length-1].toLowerCase();if(/^(us|usa|united states)$/.test(c))cc='US';else if(/^(ca|canada)$/.test(c))cc='CA';else if(/^(uk|gb|united kingdom)$/.test(c))cc='GB';else if(/^(au|australia)$/.test(c))cc='AU';}}if(parts.length===1){const m=s.match(/^(.+?)\s+([A-Za-z]{2})$/);if(m){const a=resolveAdmin(m[2]);if(a){city=m[1].trim();admin=a.admin;cc=a.cc;}}else{const low=s.toLowerCase();const names=Object.keys(US_NAME).concat(Object.keys(CA_NAME)).sort((a,b)=>b.length-a.length);for(const n of names){if(low.endsWith(' '+n)){city=s.slice(0,-(n.length+1)).trim();const a=resolveAdmin(n);if(a){admin=a.admin;cc=a.cc;}break;}}}}const variants=[];if(city)variants.push(city);if(s&&s!==city)variants.push(s);if(city&&admin)variants.push(city+' '+admin);return{city,admin,cc,variants:[...new Set(variants.filter(Boolean))]};}
...
el('btn-refresh')&&(el('btn-refresh').onclick=()=>refresh({force:true,packFirst:true,boot:false}));
...
if(!isMobileUI())setTimeout(()=>scheduleForecast(false),2500);
})();

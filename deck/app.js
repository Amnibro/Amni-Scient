...
tDraw()
Promise.all([wasmReady, catReady]).then(() => { buildMats(); initUI(); recompute(); tick(); maybeScanBanner() })
...
async function mountDraw() {
  ...
  try { const m = await import('./shape-edit.js?v=se2'); shapeEd = m.mountShapeEditor(host, { rect: { w: +cfg.length, d: +cfg.depth }, polygon: cfg.mode === 'poly' ? cfg.polygon : null, houseEdge: cfg.house_edge, onApply: ({ polygon, houseEdge }) => { applyPoly(polygon, houseEdge); const t = document.querySelector('.tab[data-pane="3d"]'); if (t) t.click() } }) }

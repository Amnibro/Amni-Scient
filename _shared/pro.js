...
<h3>Labor</h3><div id="pro-labor"></div><div style="display:flex;gap:6px;flex-wrap:wrap"><button class="pro-btn ghost" id="pro-labor-add">+ Add labor line</button><select id="pro-ratebook" style="flex:1;min-width:150px;background:var(--bg,#0b0e12);border:1px solid var(--line,#2a3038);border-radius:6px;color:var(--mut,#8a94a0);font:inherit;font-size:11.5px;padding:6px"><option value="">+ from rate book…</option></select><button class="pro-btn ghost" id="pro-rb-edit" title="Edit rate book" style="font-size:11px;padding:6px 10px">✎</button></div><div id="pro-rb-panel" style="display:none;margin-top:8px;border:1px dashed var(--line,#2a3038);border-radius:8px;padding:8px"></div>
...
const rbFill = () => { rb.innerHTML = '<option value="">+ from rate book…</option>' + PS.rateBook.map((e, i) => `<option value="${i}">${e.d} — $${e.r}/hr</option>`).join('') }
rbFill()
...
const rbUI = () => { const w = S('pro-rb-panel'); w.innerHTML = '<label style="margin-bottom:6px">Your rate book (used by every quote)</label>'; PS.rateBook.forEach((e, i) => { const row = document.createElement('div'); row.className = 'pro-labor'; row.innerHTML = '<input type="text" class="l-desc"><input type="number" class="l-rate" step="1" style="width:74px"><button class="l-del" title="Remove">✕</button>'; const [de, ra] = row.querySelectorAll('input'); de.value = e.d; ra.value = e.r; de.addEventListener('input', () => { e.d = de.value; saveP(); rbFill() }); ra.addEventListener('input', () => { e.r = +ra.value || 0; saveP(); rbFill() }); row.querySelector('.l-del').onclick = () => { PS.rateBook.splice(i, 1); saveP(); rbFill(); rbUI() }; w.appendChild(row) }); const add = document.createElement('button'); add.className = 'pro-btn ghost'; add.textContent = '+ Add task'; add.style.fontSize = '11px'; add.onclick = () => { PS.rateBook.push({ d: '', r: Q.r.rate || 75 }); saveP(); rbFill(); rbUI() }; w.appendChild(add) }
S('pro-rb-edit').onclick = () => { const w = S('pro-rb-panel'); const on = w.style.display !== 'none'; w.style.display = on ? 'none' : 'block'; on || rbUI() }

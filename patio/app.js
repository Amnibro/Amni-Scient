...
import { initPermits, updatePermits } from './codes.js?v=114'
import { initMapTrace, sitePlanSVG, cropForPlan, mapPlanSnapshot } from './maptrace.js?v=114'
...
const wasm = await WebAssembly.instantiateStreaming(fetch('patio_core.wasm?v=114'))
...
const callCore = c => {
  const payload = enc.encode(JSON.stringify({ polygon: polyOf(c), thickness_in: +c.thickness_in, base_in: +c.base_in, reinforce: c.reinforce, finish: c.finish, turndown: c.turndown, vehicle: c.vehicle, joint_max_ft: +c.joint_max_ft, house_edge: c.mode === 'rect' ? +c.house_edge : +c.house_edge }))

...
const rebuild3D = () => {
  ...
  { const xs = poly.map(p => p[0]), ys = poly.map(p => p[1]), minX = Math.min(...xs), maxX = Math.max(...xs), minY = Math.min(...ys), maxY = Math.max(...ys), sy = b + t, dy = maxY - minY, dx2 = maxX - minX
    cfg.pos = cfg.pos || {}; const P = (k, dx, dz) => { const ov = cfg.pos[k]; return [ov ? ov[0] : dx, ov ? ov[1] : dz, ov ? ov[2] : null] }
    if (dy > 7 && dx2 > 7) { const cx = (minX + maxX) / 2, tz = -(maxY - dy * 0.36)
      const tp = P('table', cx, tz), tbl = makeTable(); tbl.userData.drag = { key: 'table' }; tbl.position.set(tp[0], sy, tp[1]); tbl.rotation.y = tp[2] != null ? tp[2] : 0; grp.add(tbl)
      let ci = 0; for (const [ox, oz] of [[0, 2.4], [0, -2.4], [2.4, 0], [-2.4, 0]]) { if (ci >= 3) break; const k = `chair${ci}`, cp = P(k, cx + ox, tz + oz), ch = makeChair(); ch.userData.drag = { key: k }; ch.position.set(cp[0], sy, cp[1]); ch.rotation.y = cp[2] != null ? cp[2] : Math.atan2(ox, oz) + Math.PI; grp.add(ch); ci++ }
      const p1 = P('planter1', minX + 1.4, -(minY + 1.4)), pl1 = makePlanter(0x4a8a3a); pl1.userData.drag = { key: 'planter1' }; pl1.position.set(p1[0], sy, p1[1]); pl1.rotation.y = p1[2] != null ? p1[2] : 0; grp.add(pl1)
      const p2 = P('planter2', maxX - 1.4, -(minY + 1.4)), pl2 = makePlanter(0xc24a5a); pl2.userData.drag = { key: 'planter2' }; pl2.position.set(p2[0], sy, p2[1]); pl2.rotation.y = p2[2] != null ? p2[2] : 0; grp.add(pl2)
    } }

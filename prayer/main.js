...
function generateDiscussResponse(query, idx) {
  ...
  if (q.match(/mean|what|explain|interpret|signif|symbol|understand|why|word|term|goods|treasure|wealth|spirit|soul|heart|mystery|hidden|refer|virtue|lord|god|king|law|covenant|faith|hope|love|joy|peace|fear|justice|mercy|grace/)) {
    const words = q.match(/\b([a-z]{4,})\b/g)?.filter(w=>!['what','does','this','mean','here','that','with','from','about','they','have','does','verse','text','pass','bible','book','does','make','just','also','more','some','than','your','their','these','those'].includes(w))?.slice(0,2)||[];
    const focus = words.length ? `The term <em>\u201c${words[0]}\u201d</em>` : 'This language';
    return `${ref} \u2014 <em>\u201c${escHtml(vtS)}\u201d</em><br><br>${focus} in this ${era} setting ${era==='Wisdom'?'carries compressed moral meaning \u2014 material goods, wealth, or possessions become mirrors of the soul\u2019s posture toward God. The Wisdom tradition consistently reframes outward realities as inward measures.':era==='Historical'?'operates at the surface as narrative fact, but the text insists on more: possessions, places, and events in Historical writing encode covenant standing, divine blessing, or judgment.':era==='Prophets'?'is being reclaimed by the prophetic voice \u2014 ordinary words loaded with eschatological and covenantal weight far beyond their surface meaning.':era==='Epistles'?'is doing precise theological work within a pastoral argument. Epistolary writers choose terms carefully to shape the understanding of a living community.':'merits close attention to its rhetorical function and the tradition it inhabits.'}<br><br>${bridgesT?`The ${ot} OT \xb7 ${nt} NT resonances suggest this language participates in a theme running across both Testaments. `:''}${conns.length?`Try <em>how does it connect</em> to trace that theme through ${conns.length} related verses.`:''}`;
  }
  return `${ref}${vt?` \u2014 <em>\u201c${escHtml(vtS)}\u201d</em>`:''}<br><br>What stands out: ${era==='Wisdom'?'the compression of moral truth into memorable form \u2014 every word load-bearing.':era==='Historical'?'the way events become theology \u2014 the narrative insists on meaning behind the facts.':era==='Prophets'?'the prophetic voice wedged between divine address and human response \u2014 the tension is the point.':era==='Epistles'?'the pastoral precision \u2014 doctrine shaped by the needs of a real community.':'the layered intention of the text.'} ${bridgesT?`The OT\u2013NT bridge it forms points to a typological pattern worth tracing. `:``}${ppl.length?`<br><br>Try: <em>who is ${ppl[0]} here?</em> `:''}${locs.length?`Or: <em>where does ${locs[0]} figure into the larger story?</em>`:`Or: <em>how does this connect across the canon?</em>`}`;
}

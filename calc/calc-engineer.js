...
var DESC={beam:'DESIGNER: load + span + sag limit → required I & S — plus full shear/moment/deflection solver',stress:'Stress tensor components → principal stresses, von Mises / Tresca, FoS against yield',section:'Shape dimensions or a drawn polygon → A, I, S, r — hands off to Beam / Shaft / Columns',bolts:'DESIGNER: count + material + load → recommended size, torque, preload & margins — or analyze a chosen bolt in full',springs:'DESIGNER: force + deflection → wire/coil/turns — plus full analysis, stacks & packs for all 5 spring types',seals:'DESIGNER: bore + pressure + fluid + temp → cross-section, groove & elastomer — plus full gland analysis',columns:'DESIGNER: load + length + fixity → required I & A — plus Euler/Johnson buckling analysis',shafts:'DESIGNER: torque + material → stock diameter — plus torsion, critical speed and key analysis',fits:'Nominal Ø + ISO 286 classes → hole/shaft limits, clearance or interference, press-fit torque capacity & assembly ΔT',welds:'DESIGNER: load + length → fillet leg size — plus throat stress and eccentric group analysis',bearings:'DESIGNER: load + speed + life target → required C — plus ISO 281 L10 analysis',gears:'DESIGNER: power + speed + ratio + hardness → module & face width — plus geometry, Lewis bending, AGMA pitting',fatigue:'Mean/alternating stress + Se → Goodman / Gerber / Soderberg safety factors; Marin Se builder',vibration:'DESIGNER: mass + disturbing Hz + isolation % → mount stiffness — plus SDOF, beam modes, resonance',fluids:'DESIGNER: flow + run + ΔP budget → Sch 40 pipe size — plus Reynolds, Moody friction, minor losses, Bernoulli',pumps:'DESIGNER: flow + head → shaft power, motor size & pump type — plus NPSH, affinity, specific speed',hydraulics:'DESIGNER: force + pressure + stroke → cylinder bore & rod with buckling check — plus forces/speeds and accumulator sizing',machining:'Speeds & feeds from published bands, tap drills by % thread, bend allowance & flat pattern, E140 hardness, bolt-circle coordinates',cad:'Parametric CSG modeler — stack primitives, cut holes, exact mass properties, export watertight STL for printing or CAM',rigging:'Sling tension by angle & hitch, CoG-offset leg loads, eyebolt derates — with the 30° refusal every rigger enforces',thermal:'Geometry + properties → conduction / convection / radiation, fins, h from Nu, transients',hx:'DESIGNER: duty + temperatures → required area — plus LMTD (F-factor), ε-NTU and fouled-U analysis',pv:'Pressure + geometry → thin-wall / Lamé / ASME VIII-1 wall stress and required thickness',cycles:'Cycle parameters → Carnot / Otto / Diesel / Brayton efficiency, refrigeration COP',hvac:'Air states + loads → psychrometrics, cooling load, duct sizing',combustion:'Fuel composition → stoichiometric AFR, λ, adiabatic flame temperature, LHV',electrical:'V / I / R + phases → Ohm, AC power triangle, wire drop, RC/RL time constants',motors:'DESIGNER: torque + speed → kW/HP rating, poles, design letter & frame — plus torque, FLA, slip, service factor',nec:'DESIGNER: load + run → smallest AWG passing ampacity & drop — plus derated ampacity analysis',echem:'Electrode & cell parameters → Nernst, Tafel, Butler-Volmer, corrosion rate, Faraday mass',battery:'DESIGNER: target V + Ah → SxP topology — plus runtime with Peukert and state of charge',materials:'Searchable database — ~190 metals, alloys and polymers with mechanical & thermal data',finishes:'Weighted requirements → ranked surface finish / coating recommendations',math:'Quadratics, cubics, linear systems, geometry and calculus utilities with shown work',equations:'Formula reference cards for every module',units:'Unit converter across all engineering dimensions',refs:'Standards and literature the calculators implement'};

...
const NOUN = { deck: 'deck', patio: 'patio concrete', pool: 'swimming pool', floor: 'flooring', roof: 'roof reroof', frame: 'framing addition', plumb: 'plumbing', elec: 'electrical', plan: 'building zoning', garden: 'zoning', hvac: 'mechanical hvac' }
...
const TRADES = {
  ...
  hvac(cfg, out, frost, rows) {
    rows.push(box('', `A mechanical (HVAC) permit + rough-in (duct) and final inspection are typically required. Work to the IMC or the IRC mechanical chapters your state adopts (most on the 2018/2021 editions).`))
    rows.push(box('info', `Size by the BOOKS, not rules of thumb: ACCA Manual J load calc → Manual S equipment selection → Manual D duct design. Many jurisdictions now require the Manual J on submittal.`))
    rows.push(box('info', `Ducts in unconditioned space must be sealed with mastic (not cloth tape) and insulated (R-6 / R-8), and every system needs adequate return air. Combustion appliances need proper venting + combustion air.`))
    rows.push(box('info', `A new condenser / line-set also needs an electrical permit (disconnect + dedicated circuit). Refrigerant and gas tie-ins usually require a licensed mechanical contractor (EPA 608).`))
  },
}
const render = () => {

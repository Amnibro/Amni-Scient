...
const shingleTex = (hex, kind) => mkTex(1, g => { const b = new THREE.Color(hex), hx = l => `#${b.clone().offsetHSL(0, 0, l).getHexString()}`; g.fillStyle = hx(-0.04); g.fillRect(0, 0, 256, 256); if (kind === 'metal') { for (let x = 0; x < 256; x += 26) { g.fillStyle = hx(0.08); g.fillRect(x, 0, 2.5, 256); g.fillStyle = hx(-0.13); g.fillRect(x + 2.5, 0, 1.5, 256) } return } const courses = kind === '3tab' ? 12 : 11, ch = 256 / courses, tw = kind === '3tab' ? 26 : 34; for (let r = 0; r < courses; r++) { const y = r * ch, off = (r % 2) * (tw / 2); for (let x = -tw; x < 256; x += tw) { g.fillStyle = hx((Math.random() - 0.5) * (kind === '3tab' ? 0.05 : 0.1)); g.fillRect(x + off, y, tw - 1, ch); g.strokeStyle = 'rgba(0,0,0,0.09)'; g.lineWidth = 1; g.strokeRect(x + off + 0.5, y + 0.5, tw - 2, ch - 1) } const gr = g.createLinearGradient(0, y + ch * 0.6, 0, y + ch); gr.addColorStop(0, 'rgba(0,0,0,0)'); gr.addColorStop(1, 'rgba(0,0,0,0.24)'); g.fillStyle = gr; g.fillRect(0, y + ch * 0.6, 256, ch * 0.4) } for (let i = 0; i < 1400; i++) { g.fillStyle = `rgba(${Math.random() > 0.5 ? '255,255,255' : '0,0,0'},${Math.random() * 0.05})`; g.fillRect(Math.random() * 256, Math.random() * 256, 1.4, 1.4) } })
...
async function mountDraw() {
  ...
  try { const m = await import('./shape-edit.js?v=se3'); shapeEd = m.mountShapeEditor(host, { rect: { w: +cfg.w, d: +cfg.d }, polygon: cfg.mode === 'poly' ? cfg.polygon : null, houseEdge: cfg.house_edge, onApply: ({ polygon, houseEdge }) => { applyPoly(polygon, houseEdge); const t = document.querySelector('.tab[data-pane="3d"]'); if (t) t.click() } }) }
